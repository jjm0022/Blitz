...
class Connection:
    ...
    def total_not_in_dataset(self):
        return self.session.query(Status).\
            filter(Status.in_dataset == 0).\
            count()

    @property
    def total_in_dataset(self):
        return self.session.query(Status).\
            filter(Status.in_dataset == 1).\
            count()

    def get_not_in_dataset(self, limit=1000):
        '''
        '''
        unprocessed = self.session.query(Status).filter(
            Status.in_dataset == 0).limit(limit)
    ...
    def get_forecast_extractions(self, forecast):
        '''
        '''
        extractions = self.session.query(Extraction).filter(
            Extraction.forecast_id == forecast.id).all()
        ...
        for status in self.get_not_in_dataset(limit=limit):
            ...
            try:
                ...
                for extraction in self.get_forecast_extractions(forecast):
                    annotation = self.Annotation(start=extraction.start_index,
